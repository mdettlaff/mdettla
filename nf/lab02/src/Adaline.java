/*
 * Algorytm ADALINE.
 * Punkty leza wzdluz prostej i ADALINE szuka rownania tej prostej.
 */

class Adaline {
//	String f_n="d:\\Java\\Nauka\\Fuzzy\\FClustering\\p01\\Data\\";
	String f_name = "Ada.txt"; //f_n+"data3_2.txt"; //"iris.txt"; //"iris.txt";
	...
	/**
	 * Tylko ta metoda rozni sie w porownaniu do perceptronu (i warunek stopu).
	 */
    int singleEpoch() {
    	double output;
    	int ile=0;
    	for (int i=0; i<m; i++) {
    		output = b;
    		for (int j=0; j<n; j++) {
    			output += w[j]*x[perm[i]][j];
    		}
			int error = 0;
			for (int j = 0; j < n; j++) {
				// TODO
				error += Math.sqr(y[perm[i]] - 0);
			}
			boolean isClassifiedCorrectly = error < 0.1;
    		if (!isClassifiedCorrectly) {
    			for (int j=0; j<n; j++) {
...
//		System.out.println("testuj");
		...
		//randPerm(); w ADALINE nie jest potrzebna permutacja
		do {
			changes=singleEpoch();
