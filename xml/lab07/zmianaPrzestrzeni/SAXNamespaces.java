...
//			parser.setErrorHandler(errorHandler);
			ChangeNamespaceFilter filtr=new ChangeNamespaceFilter();
			filtr.setParent(parser);
			filtr.setContentHandler(contentHandler);
			filtr.setErrorHandler(errorHandler);
			filtr.parse(uri);
//			parser.parse(uri);
			if (parser.getFeature("http://xml.org/sax/features/namespaces")){
				System.out.println("Z przestrzeniami nazw");
			}else{
				System.out.println("Bez przestrzeni nazw");
			}
	...
	public static void main(String[] args){
		...
		String uri=args[0];
		SAXNamespaces parserDemo=new SAXNamespaces();
