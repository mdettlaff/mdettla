...
/**
 * Test szybkosci pisania na klawiaturze. Wyniki przesylane sa do skryptu PHP.
 *
...
public class TypingTest extends Applet
  ...
  final int MAX_LINES=8; // ile pelnych wierszy sie zmiesci na ekranie
  ...
  static String jarFileName; // archiwum z apletem i plikami do przepisywania
  ...
  public void init() {
    ...
    mistakesCount=0;
    maxCharsInLine=65;
    jarFileName = "TypingTest.jar";
    loadNewTextToType(jarFileName);
  ...
  void drawInBuffer() {
	...
	buffer.drawString("Wynik: " + String.format("%.1f", speed) +
	  " znaków/min, " + mistakesCount + " b³êdów (" + correctCharsTyped +
  ...
  public void keyTyped(KeyEvent e) {
	...
	if (typedText.getLast().length() != 0) {
	  String s = typedText.getLast();
	  s = s.substring(0, s.length()-1); // bez ostatniego znaku
	  typedText.setLast(s);
	  charsTyped--;
	  ...
	  s = s.substring(0, s.length()-1); // bez ostatniego znaku
	  mistakes.setLast(s);
	} else if (typedText.size() > 1) { // wroc do poprzedniej linii
	  mistakes.removeLast();
	  typedText.removeLast();
	  charsTyped--; // odejmujemy enter, ktory wpisalismy wczesniej
	  // wymazujemy podkreslenia z konca linii do ktorej wracamy
	  String s;
	  while ((s = typedText.getLast()).length() > 0
	...
	if (charsTyped > 0 || isTimerOn) {
	  ...
	  if (typedText.getLast().length() < text.get(
	    ...
	    typedText.setLast(typedText.getLast().concat(underline));
	  }
	  ...
	  if (typedText.size() > text.size()) { // jesli to byla ostatnia linia
	    ...
	    // obliczenie wynikow testu
	    long typingTime = timeFinished - timeStarted; // w milisekundach
	    double tt = ((double) typingTime) / 1000;
	    // przy mierzeniu predkosci bierzemy pod uwage tylko poprawne znaki
	    correctCharsTyped = charsTyped - mistakesCount;
	    double cct = correctCharsTyped;
	    speed = 60 * cct / tt;
	    long typingTimeS = typingTime / 1000; // w sekundach
	    minutes = (int) typingTimeS / 60;
	    seconds = (int) typingTimeS % 60;

	    // zapisanie wynikow w pliku tekstowym przy pomocy skryptu PHP
	    try {
	      URL url = new URL(getCodeBase().toString()
		  + "/ttlog.php?speed=" + String.format("%.1f", speed)
		  + "&mistakes=" + mistakesCount
		  + "&plChars=" + showPolishChars
		  + "&correctChars=" + correctCharsTyped
		  + "&minutes=" + minutes + "&seconds=" + seconds);
	      URLConnection urlConnection = url.openConnection();
	      // ponizsza metoda jest konieczna do odpalenia skryptu PHP
  ...
  /** Wczytuje nowy losowo wybrany plik tekstowy z danego archiwum JAR. */
  void loadNewTextToType(String jarFileName) {
    Text txtFiles = new Text();
    ...
    try {
      ...
      JarEntry jarEntry;
      while ((jarEntry = (JarEntry) jarFile.getNextJarEntry()) != null) {
	if (jarEntry.getName().matches(".*\\.txt")) {
	  txtFiles.add(jarEntry.getName());
	}
      }
    } catch (java.io.IOException e) { }

    text = new Text();
    currentTxtFile = txtFiles.get(rand.nextInt(txtFiles.size()));
    read(text, currentTxtFile);
    text.breakLines(maxCharsInLine);
    if (!showPolishChars) {
      text.removePolishChars();
  ...
  public String getAppletInfo() {
    return "Test prêdkoœci pisania na klawiaturze.";
  }
}

/**
 * Tekst w postaci kolejnych linii umieszczonych w strukturze LinkedList
 */
class Text extends LinkedList<String> {
