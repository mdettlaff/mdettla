...
/* tag dla appletviewera; otworz aplet komenda appletviewer Maze.java
...
public class Maze extends Applet implements Runnable {
  ...
  class MazeCell {
    ...
    boolean isSolutionIn() {
      if (solution.north || solution.south ||
    ...
    // przy okazji odznacza solution, bo komorka nie moze byc dwoma naraz
  ...
  public void run() {
	...
	while (visitedCells < totalCells) {
	  ...
	  Coords[] intactNeighbors = new Coords[4]; // jest 4 sasiadow
	...
	/* Algorytm Tremaux znajdowania wyjscia z labiryntu */

	currentCell = new Coords(entry.x, entry.y);
	boolean endTremaux=false;

	while (!endTremaux) {
	  // znajdujemy wolnych sasiadow, bez sciany, bactrack i solution
	  // oraz sasiadow, bez sciany, backtrack, ale z solution
	  Coords[] freeNeighbors = new Coords[4]; // jest 4 sasiadow
	  Coords[] neighborsWithSolution = new Coords[4];
	  int i;
	  int freeNborsCount; // number of free neighbors
	  int nborsWithSolCount; // number of neighbors with solution
	  for (i=0, freeNborsCount=0, nborsWithSolCount=0; i < 4; i++) {
	    int x=DIRECTIONS[i].x+currentCell.x;
	    int y=DIRECTIONS[i].y+currentCell.y;
	    if (!isBorderInDirection(currentCell, DIRECTIONS[i])) {
	      if (!isWallInDirection(currentCell, DIRECTIONS[i]) &&
		!maze[x][y].isBacktrackIn() && !maze[x][y].isSolutionIn()) {
		freeNeighbors[freeNborsCount] = new Coords(x, y);
		freeNborsCount++;
	      }
	      else if (!isWallInDirection(currentCell, DIRECTIONS[i]) &&
		!maze[x][y].isBacktrackIn()) {
		neighborsWithSolution[nborsWithSolCount] = new Coords(x, y);
		nborsWithSolCount++;
	      }
	    }
	  }
	  ...
	  if (currentCell.x == exit.x && currentCell.y == exit.y)
	    endTremaux=true;
	  else if (freeNborsCount > 0) { // jesli obok jest wolna komorka
	    // idziemy w kierunku nowej komorki znaczac solution
	    int randInd=rand.nextInt(freeNborsCount); // random index
	    Coords newCell = new Coords(freeNeighbors[randInd].x,
					freeNeighbors[randInd].y);
	    maze[currentCell.x][currentCell.y].markSolution(
					    currentCell, newCell);
	    currentCell = newCell;
	  }
	  // jesli jest obok komorka z solution do ktorej mozna pojsc...
	  else if (nborsWithSolCount > 0) {
	  // ...idziemy tam znaczac po drodze backtrack
	    int randInd=rand.nextInt(nborsWithSolCount);
	    Coords newCell = new Coords(neighborsWithSolution[randInd].x,
			neighborsWithSolution[randInd].y);
	    maze[currentCell.x][currentCell.y].markBacktrack(
			      currentCell, newCell);
	    currentCell = newCell;
	  }
	  else // nie mozna juz isc dalej; albo znalezlismy wyjscie, albo
	    endTremaux=true; // wrocilismy do poczatku labiryntu
  ...
  // sprawdza, czy jest sciana w danym miejscu komorki
  boolean isWallInDirection(Coords cell, Coords direction) {
    if (direction.y < 0 && maze[cell.x][cell.y].walls.north == true)
      return true; // jest sciana na polnoc
    if (direction.y > 0 && maze[cell.x][cell.y].walls.south == true)
      return true; // jest sciana na poludnie
    if (direction.x > 0 && maze[cell.x][cell.y].walls.east == true)
      return true; // jest sciana na wschod
    if (direction.x < 0 && maze[cell.x][cell.y].walls.west == true)
      return true; // jest sciana na zachod
    return false;
  }

  // sprawdza, czy jest brzeg w danym miejscu komorki
  boolean isBorderInDirection(Coords cell, Coords direction) {
  ...
  public void drawInBuffer() {
    final int P_SIZ=5; // how thin solution and backtrack paths are
