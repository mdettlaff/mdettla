...
class Biomorph {
  ...
  static final double MAX_EXPAND = 1.8;
  ...
  static final Biomorph getDefault() {
    return new Biomorph(45, 45, 18, 18, 1.0, 1.0, 5, 0.75,
       	Biomorph.INIT_COLOR);
  }

  static Biomorph getRandom() {
    int frontAngle = Biomorphs.rand.nextInt(360);
    int rearAngle = Biomorphs.rand.nextInt(360);
    int frontElongation = Biomorphs.rand.nextInt(MAX_ELONGATION+1);
    int rearElongation = Biomorphs.rand.nextInt(MAX_ELONGATION+1);
    double expandX = ((double) Biomorphs.rand.nextInt((int)(MAX_EXPAND*100)))
							  / 100;
    double expandY = ((double) Biomorphs.rand.nextInt((int)(MAX_EXPAND*100)))
							  / 100;
    int iterations = Biomorphs.rand.nextInt(MAX_ITERATIONS+1);
    double gradient = ((double) Biomorphs.rand.nextInt((int)(MAX_GRADIENT*100))
							  / 100);
...
class GenerationCanvas extends Canvas implements MouseListener {
   ...
   * wspolczynniki, oraz ogolnie powiekszona przez kolejny wspolczynnik.
   */
