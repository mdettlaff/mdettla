...
public class MultipassFuzzyFilter implements ImageProcessingAlgorithm {
	...
	static {
		for (int i = 0; i < 512; i++) { // TODO remove magic number
			p[i] = new int[512];
		}
	}
	...
	public BufferedImage processImage(BufferedImage inputImage) {
		...
		processedImage = new FuzzyBlock3().processImage(processedImage);
		...
		public BufferedImage processImage(BufferedImage inputImage) {
			...
			for (int y = 1; y < inputImage.getHeight() - 1; y++) {
				...
				for (int x = 1; x < inputImage.getWidth() - 1; x++) {
					...
					q = q + deltaR;
					p[y][x] = Math.abs(deltaR); // used by fuzzy block 3
	...
	private static class FuzzyBlock3 implements ImageProcessingAlgorithm {
		...
		public BufferedImage processImage(BufferedImage inputImage) {
			...
			for (int y = 1; y < inputImage.getHeight() - 1; y++) {
				...
				for (int x = 1; x < inputImage.getWidth() - 1; x++) {
					int q = Util.q(x, y, inputImage);
					if (p[y][x] > 0) {
						int deltaZ = (int)deltaZ(x, y, inputImage);
						q = q + deltaZ;
						q = Math.max(0, q);
						q = Math.min(L - 1, q);
						processedImage.setRGB(
								x, y, new Color(q, q, q).getRGB());
					} else {
						processedImage.setRGB(
								x, y, new Color(q, q, q).getRGB());
					}
		...
		private static double deltaZ(int x, int y, BufferedImage image) {
			List<Double> arr0 = new ArrayList<Double>();
			Integer [][] deltaW = deltaW(x, y, image);
			for (int j = 0; j < W.length; j++) {
				List<Double> arr00 = new ArrayList<Double>();
		...
		/**
		 * Membership function of the fuzzy set MEDIUM.
		 */
		private static double membershipMedium(int u) {
			final int a1 = 90;
			final int a2 = 60;
			if (0 <= u && u < (L - 1) / 2 - a1) {
				return 0;
			} else if ((L - 1) / 2 - a1 <= u && u < (L - 1) / 2 - a2) {
				return (((double)u) + a1 - (L - 1) / 2) / (a2 - a1);
			} else if ((L - 1) / 2 - a2 <= u && u < (L - 1) / 2 + a2) {
				return 1;
			} else if ((L - 1) / 2 + a2 <= u && u < (L - 1) / 2 + a1) {
				return (((double)u) - a1 - (L - 1) / 2) / (a1 - a2);
			} else if ((L - 1) / 2 + a1 <= u && u < (L - 1)) {
				return 0;
			} else {
				throw new AssertionError("nieprawidłowa wartość u: " + u);
			}
