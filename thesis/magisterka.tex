...
[monografy i diagrafy, statystyka tekstu zamiast analizy całego zbioru tekstów]
...
$$ p_{finger} = \sum\limits_{monographs} (f_{opt} - f) $$
...
$$ p_{row} = \sum\limits_{monographs} (f_{opt} - f) $$
...
$$ p_{hand_alter} = \sum\limits_{diagraphs} f_{sh} $$
...
$$ p_{finger_alter} = \sum f_d dist(d) $$
