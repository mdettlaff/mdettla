...
class greedyRN{
					...
					*/
					...
					for (int j=0; j<odl.length; j++) {
						if (odl[node.miasto][j]>0) {
							g_tmp = node.g+odl[node.miasto][j];
							f_tmp = g_tmp + h[j];
							//czy istnieje "lepsza" droga przez j
							tmp = better(j,f_tmp);
							if (tmp==(int)Double.MAX_VALUE) {
								node1=new Node(j,node.miasto,g_tmp,h[j]);
								open.add(node1);
							} else
							if (tmp>=0 && tmp<=f_tmp) {
								open.remove(tmp);
	...
	void piszTrase(int city, int start) {
		boolean found, pisano=false;
		int i;
		Node node;
			...
			while (city!=start) {
				...
				do {
					...
					if (node.miasto==city) {
						found=true;
						if (!pisano) {
							System.out.println("Total length: "+node.g);
	...
	void pisz(String s,LinkedList lista) {
		...
		for (int i=0;i<lista.size(); i++) {
			...
			if (nd.przez>=0) System.out.println(name[nd.przez]+") "+nd.g+" "+nd.h+" "+nd.f);
			else
