 ...
 * Przykład z książki T.J. Ross "Fuzzy Logic with Engineering Applications"
 *
...
class PendulumD {
	...
	double INFTY = 1000;
	int[][] FAM = {{0,1,2}, {1,2,3}, {2,3,4}}; //baza reguł
	
	int u_ile = 5;//liczba wartości przyjmowanych przez sterowanie
	double u_mn = -24;
	double u_mx =  24;
	...
	/*
	 * Trapezoidalna funkcja przynalezności
	 * Nie używana
	 */
	double trapez(double x, double a, double b, double c, double d) {
		if (x<=a|| x>= d)
			return 0.0;
		else
		if (a==-INFTY && x <= b)
	...
	/*
	 * bada stopień zgodności obserwacji z przesłankami kolejnych reguł
	 */
